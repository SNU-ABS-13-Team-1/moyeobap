...
const ROLE_LABEL: Record<ChessChatAuthorRole, string> = {
  white: '백',
  black: '흑',
  spectator: '관전',
};
...
export function ChessChat({ roomId, myRole }: { roomId: string; myRole: ChessChatAuthorRole }) {
  ...
  const { data, error, mutate } = useSWR<{ messages: ChessChatMessage[] }>(
    `/api/games/chess/rooms/${roomId}/chat`,
    fetcher,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chess-chat-${roomId}`)
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'chess_chat_messages', filter: `room_id=eq.${roomId}` },
        () => mutate(),
      )
      .subscribe();
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const optimisticMsg: ChessChatMessage = {
      id: `temp-${Date.now()}`,
    ...
    setSending(true);
    try {
      await requestJson(`/api/games/chess/rooms/${roomId}/chat`, {
        method: 'POST',
