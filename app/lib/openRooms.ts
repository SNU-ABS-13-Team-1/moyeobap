...
/** 바둑은 오목과 같습니다 — 자리 두 칸, 방장이 흑. 계가 중(scoring) 상태가 있습니다. */
export function fromBaduk(room: OmokLike): OpenRoom {
  const seated = [room.blackId, room.whiteId].filter((id): id is string => Boolean(id));
  return {
    ...base("baduk", room.id),
    id: room.id,
...
export function fromRummy(room: RummyLike): OpenRoom {
  ...
  return {
    ...
    hasOpenSeat: room.status === "waiting" && here.length < RUMMY_MAX_PLAYERS,
    hostId: room.hostId,
...
export function fromPhone(room: PhoneLike): OpenRoom {
  ...
  return {
    ...
    hasOpenSeat: room.status === "waiting" && here.length < PHONE_MAX_PLAYERS,
    hostId: room.hostId,
    memberIds: here.map((player) => player.id),
