...
import { getRoom } from "./alkkagi";

export type AlkkagiChatAuthorRole = "black" | "white" | "spectator";
export type AlkkagiChatKind = "text" | "image";

export type AlkkagiChatMessage = {
  id: string;
  authorId: string;
  authorName: string;
  /** 메시지를 쓴 시점의 역할. 이 기능 이전 메시지는 null입니다. */
  authorRole: AlkkagiChatAuthorRole | null;
  text: string;
  kind: AlkkagiChatKind;
  imageUrl?: string;
  createdAt: string;
};
...
type AlkkagiChatRow = {
  ...
  author_id: string;
  author_name: string;
  author_role: AlkkagiChatAuthorRole | null;
  text: string;
  kind: AlkkagiChatKind;
  created_at: string;
};
...
function mapRow(row: AlkkagiChatRow): AlkkagiChatMessage {
  return {
    id: row.id,
...
export async function getRoomChat(roomId: string): Promise<AlkkagiChatMessage[]> {
  const supabase = getSupabase();
  if (!supabase) return [];

  const { data, error } = await supabase
    .from("alkkagi_chat_messages")
  ...
  return (data as AlkkagiChatRow[]).map(mapRow);
}
...
export async function postRoomChat(
  ...
  userName: string,
  text: string,
  kind: AlkkagiChatKind = "text",
): Promise<AlkkagiChatMessage | { error: string }> {
  ...
  const authorRole: AlkkagiChatAuthorRole =
  ...
  return mapRow(data as AlkkagiChatRow);
}
