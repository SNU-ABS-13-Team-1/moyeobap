...
test("제한 시간을 막 넘겼을 때는 유예 시간 안이라 아직 초과가 아니다", () => {
  // 클라이언트 카운트다운이 0이 되는 순간 바로 호출해도, 시계 오차 때문에
  // 서버가 곧바로 패배 처리하지는 않습니다.
...
test("제한 시간 + 유예를 넘기면 시간 초과다", () => {
  ...
  assert.equal(isTurnExpired(at(0), T0 + 60_000), true);
});

test("turn_started_at이 없는 방(마이그레이션 이전 데이터)은 시간 초과로 보지 않는다", () => {
