...
import type { AlkkagiRoom } from "./alkkagi";
...
/**
 * 대국이 끝난 직후(정상 승부, 무승부, 기권, 몰수승 전부 포함) 한 번
 * 호출됩니다. 전적을 한 줄 기록하고, 표준적인 K=32 ELO 공식으로 두 사람의
 * Rating을 함께 갱신합니다.
 *
 * 무승부는 마지막 돌끼리 같이 떨어진 경우입니다(알까기에만 있는 결과).
 */
export async function recordMatchResult(
  ...
  const { error: matchError } = await supabase.from("alkkagi_matches").insert({
    room_id: room.id,
    ...
    winner,
    // 끝났을 때 남은 돌 수. 나중에 전적을 읽을 때 얼마나 아슬아슬했는지 보입니다.
  ...
  const { error: ratingError } = await supabase.from("alkkagi_ratings").upsert(
    [
      {
