...
1. **매장 데이터** — 음식점명, 위치, 메뉴 등 팀이 밖에서 조사해 넣은 매장 정보
2. **팟 데이터** — 지금 열려 있는 팟의 음식점, 참여 인원, 마감 시간
3. **기록 데이터** — 마감된 팟이 남긴 생성·참여·취소·성공·평가 이력

②와 ③은 대상이 같고 시점만 다릅니다 — 열려 있으면 팟 데이터, 마감되면 기록
데이터입니다.
